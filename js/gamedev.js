document.addEventListener('DOMContentLoaded', () => {
    fetch('/posts.json?v=' + new Date().getTime())
        ...
        .then(data => {
            const ciberData = data.filter(p => p.category === 'gamedev');

            const progress = ciberData.filter(p => p.subcategory === 'projects');
            const prominent = ciberData.filter(p => p.subcategory === 'tutorials');
            const finished = ciberData.filter(p => p.subcategory === 'videogames');
            const thoughts = ciberData.filter(p => p.subcategory === 'thoughts')
